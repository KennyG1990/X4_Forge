            ...
            <div className="flex-1 p-4 overflow-y-auto bg-black/35 font-mono text-[11px] leading-normal selection:bg-cyan-500/25 text-slate-400 select-text">
              <pre className="whitespace-pre">
                {LUA_SNIPPETS.some(sn => sn.id === selectedLuaTemplate) ? (
                  ...
                  `-- X4 standalone menu — generated by X4 Forge. Target: ui/<id>_menu.lua
-- Renders via the PROVEN pattern (verified in-game on the AI-Influence chat window + against 24
-- vanilla menus harvested from the game .cat/.dat): read Helper LAZILY (it is nil at file load),
-- register the menu the moment Helper exists (DEFERRED, inside open()), then OpenMenu(name) — the
-- engine then calls onShowMenu -> Helper.createFrameHandle -> frame:display(). Caching Helper at
-- load OR registering at file scope silently breaks rendering (X4_STANDALONE_MENU_SCHEMA).
...
local menu = {
  name = "${(workspace.name || 'Custom_Menu').replace(/[^A-Za-z0-9_]/g, '_')}",
  layer = 4,
  active = false,
}

local function log(m) if DebugError then DebugError("[${(workspace.name || 'Menu').replace(/[^A-Za-z0-9_]/g, '_')}] " .. tostring(m)) end end

-- Idempotent + DEFERRED registration. Completes the engine registration (which OpenMenu needs) the
-- moment Helper is available. Called from menu.open(), by which time Helper exists.
function menu.ensureRegistered()
  ...
  log("ensureRegistered Helper=" .. tostring(Helper ~= nil) .. " registered=" .. tostring(menu._registered == true))
...
function menu.createFrame()
  refreshHelper()
  if not Helper then log("Helper nil; cannot build frame"); return end
  if menu.frame then Helper.clearDataForRefresh(menu, menu.layer) end
  local w = Helper.scaleX(${Math.max(320, ((workspace.uiWidgets || []).length ? 640 : 600))})
  local h = Helper.scaleY(${Math.max(160, (((workspace.uiWidgets || []).length) || 1) * 56 + 90)})
  local x = ((Helper.viewWidth or 1920) - w) / 2
  local y = ((Helper.viewHeight or 1080) - h) / 2
  ...
  ftable:setColWidthPercent(1, 75)
  ftable:setColWidthPercent(2, 25)
  local row
${(workspace.uiWidgets || []).filter(w => ['header','text','input','button','chat'].includes(w.type)).map(w => {
  const label = String(w.label || w.type).replace(/"/g, '');
  const id = String(w.id);
  if (w.type === 'header') return `  row = ftable:addRow(false, {})\n  row[1]:setColSpan(2):createText("${label}", Helper.headerRowCenteredProperties)`;
  if (w.type === 'text') return `  row = ftable:addRow(false, {})\n  row[1]:setColSpan(2):createText("${label}")`;
  if (w.type === 'chat') return `  -- chat/transcript area: companion logic appends to menu.transcript (a string)\n  row = ftable:addRow(false, {})\n  row[1]:setColSpan(2):createText(menu.transcript or "${label}", { wordwrap = true })`;
  if (w.type === 'input') return `  row = ftable:addRow(true, {})\n  row[1]:setColSpan(2):createEditBox({ defaultText = "${label}", maxChars = 255 })\n  row[1].handlers.onEditBoxDeactivated = function(_, t, _, ok) if ok then AddUITriggeredEvent(menu.name, "${id}", { text = t }) end end`;
  return `  row = ftable:addRow(true, {})\n  row[1]:createText("${label}", Helper.standardTextProperties)\n  row[2]:createButton({ active = true }):setText("GO", { halign = "center" })\n  row[2].handlers.onClick = function() AddUITriggeredEvent(menu.name, "${id}", { widget = "${id}" }) end`;
}).join('\n') || '  -- Add Header / Text / Field Input / Chat / Button widgets in the designer to generate rows.'}
  menu.frame:display()
  log("frame displayed")
end
...
-- NOTE: registration is DEFERRED to menu.open() (Helper is nil at file load). No file-scope init.
return menu`
                ) : selectedLuaTemplate === 'telemetry' ? (

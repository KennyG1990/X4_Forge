...
export const validatePackageReadiness = (workspace: ModWorkspace): XMLDiagnostic[] => {
  ...
  // Cues can come from the node graph OR from hand-authored MD preserved verbatim as passthrough
  // (the faithfulness guard keeps those out of the node model). Only flag "no entry point" when
  // NEITHER source has a cue — otherwise a perfectly deployable hand-authored mod false-alarms.
  const hasGraphCue = workspace.nodes.some(node => node.type === 'cue');
  const hasPassthroughCue = (workspace.passthroughFiles ?? []).some(
    file => /(^|[\\/])md[\\/].*\.xml$/i.test(file.path) && /<(?:cue|library)\b/.test(file.content ?? '')
  );
  if (!hasGraphCue && !hasPassthroughCue) {
    reports.push({
      severity: 'error',

...
import { ModWorkspace, generateMDXML, validateModWorkspace, type MDNode, type MDLink } from '../types';
import { analyzeCueLineage } from './cueLineage';
import { checkIntent, type IntentCheckSpec } from './intentCheck';
import { critiqueWorkspace, type CriticFinding } from './mdCritic';
...
export function sanitizeBlueprint(raw: unknown): ModBlueprint {
  const rawObj = isRecord(raw) ? raw : null;
  const b = emptyBlueprint(typeof rawObj?.intent === 'string' ? rawObj.intent : '');
  if (!rawObj) return b;
  b.requirements = Array.isArray(rawObj.requirements) ? rawObj.requirements.map(String) : [];
  b.implementationPlan = Array.isArray(rawObj.implementationPlan)
    ? rawObj.implementationPlan.map((entry, i: number) => {
        const s = isRecord(entry) ? entry : {};
        return {
          id: String(s.id ?? `step_${i}`),
          step: String(s.step ?? ''),
          rationale: stringProp(s, 'rationale'),
          doneCheck: stringProp(s, 'doneCheck'),
        };
      })
    : [];
  b.tasks = Array.isArray(rawObj.tasks)
    ? rawObj.tasks.map((entry, i: number) => {
        const t = isRecord(entry) ? entry : {};
        const status = t.status;
        return {
          id: String(t.id ?? `task_${i}`),
          title: String(t.title ?? ''),
          status: (['pending', 'in_progress', 'done', 'blocked'] as const).includes(status as BlueprintTaskStatus) ? status as BlueprintTaskStatus : 'pending',
          doneCheck: stringProp(t, 'doneCheck'),
          check: isRecord(t.check) ? t.check as DoneCheckSpec : undefined,
          checkPassed: !!t.checkPassed,
          blockedBy: Array.isArray(t.blockedBy) ? t.blockedBy.map(String) : undefined,
        };
      })
    : [];
  const sp = isRecord(rawObj.scratchpad) ? rawObj.scratchpad : {};
  b.scratchpad = {
  ...
  b.changelog = Array.isArray(rawObj.changelog)
    ? rawObj.changelog.map((entry) => {
        const c = isRecord(entry) ? entry : {};
        return { at: String(c.at ?? ''), entry: String(c.entry ?? ''), verdict: stringProp(c, 'verdict') };
      })
    : [];
  return b;
}
...
export function evaluateBlueprintChecks(blueprint: ModBlueprint, workspace: ModWorkspace): ModBlueprint {
  ...
  try {
    ...
    schemaOk = !diags.some((d) => d.severity === 'error');
  } catch { schemaOk = false; }
  try {
    const findings = analyzeCueLineage(workspace?.nodes || [], workspace?.links || []).findings || [];
    graphOk = !findings.some((f) => f.severity === 'error');
  } catch { graphOk = false; }
 ...
 */
export function critiqueGate(nodes: MDNode[], links: MDLink[]): { warnings: number; advisories: number; clean: boolean; findings: CriticFinding[] } {
  try {
    const { findings } = critiqueWorkspace(nodes || [], links || []);
    const warnings = findings.filter((f) => f.severity === 'warning').length;
    const advisories = findings.filter((f) => f.severity === 'info').length;
...
export function runBlueprintSelftest(): {
  ...
  const mkN = (id: string, type: MDNode['type'], xmlTag: string, props: Record<string, unknown> = {}): MDNode => ({ id, type, label: id, xmlTag, x: 0, y: 0, properties: props, propertiesSchema: [], inputs: [], outputs: [] });
  const wsFull: ModWorkspace = {
    id: 'w', name: 'W_Test', version: '1.0.0', author: '', description: '', uiWidgets: [],
    uiTheme: { backgroundColor: '', borderColor: '', accentColor: '', opacity: 1, showIcons: true },
    templates: [],
  ...
  const wsNoTrigger: ModWorkspace = { ...wsFull, nodes: wsFull.nodes.filter((n) => n.id !== 'ev_0'), links: wsFull.links.filter((l) => l.id !== 'l1') };

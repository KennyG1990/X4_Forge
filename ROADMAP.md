...
| **UI layouts** | `md_ui_layouts/<id>_ui.xml` | ❌ Fabricated | **No such folder in X4**, and `<ui_menu><theme><layout>` is an invented schema the game ignores. Real custom UI = Lua under `ui/` with a `ui.xml` index, or MD-driven overlays. DoD: either emit a real `ui/` + Lua scaffold, or drop the WYSIWYG-to-XML pretense and reframe UI as MD overlay helpers. Highest-impact correctness fix. |
...
**Reading of the table:** the *diff-based* domains (wares, jobs, patches) are structurally on the rails but emit placeholder content; the *UI* domain is the one genuinely producing output X4 won't honor; `content.xml` has a real version bug; MD is solid. So A4 isn't just "move code" — it's "move code **and** fix these per-domain correctness issues as you go," with the round-trip + XSD harness (A1/A2) as the safety net that proves each fix.

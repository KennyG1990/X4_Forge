...
|---|---:|---:|---|---|
| **Schema-Aware Mod Doctor** | Strong | 85% | `src/lib/modDoctor.ts`, **`src/lib/xsdValidate.ts` (real XSD element/attribute/enum/required validation, 1478 elements from md.xsd+common.xsd)**, `validateModWorkspace`, `validatePackageReadiness`, package diagnostics in `CodePreview`, `/api/agent/compile`, `/api/agent/package`, `/api/agent/schema`. | **2026-06-11:** real schema-backed validation now runs on MD + AI output (enum/required/unknown-attr/unknown-element with line numbers), verified zero-false-positive on a controlled sample. Remaining: extend XSD coverage to aiscripts.xsd specifically, full sequence/cardinality checks, diagnostic grouping/navigation, and richer sourceRef links into every editor tab. |
...
| **Round-Trip Import/Edit/Export** | Partial | 72% | `SyncModal`, `DirectoryExplorer`, `SourceControl`, `parseXMLToWorkspace`, **`ModWorkspace.passthroughFiles` preservation model**, **`/api/agent/mod-folder/import` (with lossiness report)**, **`/api/agent/round-trip-check` + `round-trip-selftest` harness**, `sanitizeWorkspace`, filesystem read/write endpoints, snapshots. | **2026-06-11:** passthrough/raw-file preservation now exists and is verified lossless (unmodeled files survive import→export byte-identical); folder importer + lossiness report + structural harness added. Remaining: parse more domains into editable form (t-files/diffs/aiscripts breadth), UI badges for generated/editable/partial/passthrough, and a golden corpus across several real published mods. |
| **Diff-Safe Patch Builder** | Strong | 95% | `XMLPatchSystem`, `workspace.xmlPatches`, `compileDiffDocument`, `pos` + XPath validation, XML patch wiki docs, global search over patches, target-file base XML loaders (**now incl. packed `.cat/.dat` base files via `extractBaseGameFile`**), client-side XPath match counting, and unified diff previews. | **2026-06-11:** the prior "packed cat/dat extraction (post-MVP)" gap is closed — `/api/patch/base-content` now decodes packed base files, so XPath previews work against real packed targets (e.g. `libraries/factions.xml`), not just loose files. |

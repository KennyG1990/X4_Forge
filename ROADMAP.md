...
**Tier 1 picker coverage now:** MD `create_ship`/`create_station` macro + faction, `reward_player` faction, Wares & Jobs job faction, **and XML-patch target file**. Remaining: ware production-input pickers (serialized multi-entry UI rework) and the (deferred, low-value) SQLite read-flip.

...
function buildWorkspaceFileManifest(workspaceInput: unknown): { modId: string; files: CompiledFileManifest } {
  ...
  if (settings.md) {
    // Emit at the imported file's original stem when known, so an editable MD file keeps
    // its real filename (e.g. md/ai_influence_test_chat.xml) instead of being renamed to
    // md/<modId>.xml. New mods (no mdFileStem) fall back to the mod id.
    const mdStem = (ws.mdFileStem && /^[\w.-]+$/.test(ws.mdFileStem)) ? ws.mdFileStem : modId;
    const regen = generateMDXML(ws);
    const orig = (ws as any).mdOriginal;
    if (orig && orig.content && orig.path && canonicalMd(regen) === canonicalMd(orig.content)) {
      // Unedited since import → re-emit the ORIGINAL bytes verbatim (comments, whitespace,
      // attribute order all preserved → true byte-fidelity). Any real graph edit changes the
      // canonical form and falls through to the regenerated XML below.
      files[orig.path] = orig.content;
    } else {
      files[`md/${mdStem}.xml`] = regen;
    }
  }
...
function importModFolder(absDir: string): { workspace: ModWorkspace; report: any } {
  ...
  if (mdRel) {
    try {
      const mdText = fs.readFileSync(path.join(absDir, mdRel), 'utf8');
      ...
      if (parsed && Array.isArray(parsed.nodes) && parsed.nodes.length > 0) {
        ...
        if (mdRoundTripPreservesElements(mdText, regen)) {
          baseWorkspace = parsed;
          mdOriginalText = mdText; // keep raw bytes for byte-fidelity on unedited export
        }
        // else: lossy round-trip → keep verbatim, do NOT model as editable
      }
  ...
  const ws: ModWorkspace = sanitizeWorkspace({
    ...
    ...(mdParsed && baseWorkspace ? {
      mdScriptName: baseWorkspace.name,
      mdFileStem: mdRel ? path.basename(mdRel).replace(/\.xml$/i, '') : undefined,
    } : {}),
    ...
    // Keep the editable MD file's original bytes for byte-fidelity on unedited export.
    ...(mdParsed && mdRel && mdOriginalText ? { mdOriginal: { path: mdRel, content: mdOriginalText } } : {}),
...
function compileWorkspaceToFolder(ws: any, rootPath: string, mode: 'candy' | 'store', writeSnapshots: boolean = false): string {
  ...
  // 1. content.xml — byte-fidelity (preserves <dependency> + formatting when meta unedited).
  const contentXml = contentXmlFor(modId, ws);
  ...
  // 3. MD — match buildWorkspaceFileManifest exactly (not a divergent re-derivation):
  //    byte-fidelity emit-original-when-unedited, at the file's ORIGINAL stem (not md/<modId>.xml).
  if (settings.md) {
    const mdStem = (ws.mdFileStem && /^[\w.-]+$/.test(ws.mdFileStem)) ? ws.mdFileStem : modId;
    const regen = generateMDXML(ws);
    const orig = ws.mdOriginal;
    const useOrig = !!(orig && orig.content && orig.path && canonicalMd(regen) === canonicalMd(orig.content));
    const mdRel = (useOrig && orig.path) ? orig.path : `md/${mdStem}.xml`;
    const mdDest = path.join(targetPath, mdRel);
    fs.mkdirSync(path.dirname(mdDest), { recursive: true });
    fs.writeFileSync(mdDest, useOrig ? orig.content : regen);
  }
